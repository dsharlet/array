...
NDARRAY_UNIQUE void for_each_index_in_z_order(const Extents& extents, const Fn& fn) {
  ...
  internal::for_each_index_in_z_order_impl(end, z, Rank - 1, step, [&](const std::array<index_t, Rank>& i) { fn(i); });
}
...
NDARRAY_UNIQUE void for_each_in_z_order_impl(
  ...
  for_each_index_in_z_order(extents, [fn = std::move(fn), &ranges](const auto& i) {
