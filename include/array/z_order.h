...
NDARRAY_UNIQUE void for_each_index_z_order_impl(const std::array<index_t, Rank>& end,
  ...
  if (dim == 0 && step == 1) {
    ...
    for_each_index_z_order_impl(end, z, Rank - 1, step >> 1, fn);
    z[0] += step;
    if (z[0] < end[0]) for_each_index_z_order_impl(end, z, Rank - 1, step >> 1, fn);
    z[0] -= step;
  } else {
    // Move to the next dimension for the same step.
    for_each_index_z_order_impl(end, z, dim - 1, step, fn);
    z[dim] += step;
    if (z[dim] < end[dim]) for_each_index_z_order_impl(end, z, dim - 1, step, fn);
    z[dim] -= step;
  }
}
...
NDARRAY_UNIQUE void for_each_z_order_impl(
  ...
  for_each_index_z_order(extents, [fn = std::move(fn), &ranges](const auto& i) {
